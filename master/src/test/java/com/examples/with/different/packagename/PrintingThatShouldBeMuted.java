...
public class PrintingThatShouldBeMuted{
	...
	public void doPrint() { //throws RuntimeException{
		...
		Thread k = new Thread(){
			@Override
			public void run(){
				System.err.println("Going to sleep for a bit. SmartUt should throw a timeout");
				System.err.println("We enter in infinite loop, even if SmartUt try to interrupt");
				while(true){
					try {
						long time = 200;
