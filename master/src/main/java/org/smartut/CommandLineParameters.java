...
import org.smartut.executionmode.*;
...
public class CommandLineParameters {
	...
	public static Options getCommandLineOptions() {
		...
		Option smartutCP = new Option("smartutCP", true,
				"classpath of SmartUt jar file(s). This is needed when SmartUt is called in plugins like Eclipse/Maven");
		...
		Option jvm_debug = new Option("jvm_debug", true, "local plugin mvn debug");
	...
	public static void addJavaDOptions(List<String> javaOpts, CommandLine line) throws Error {
        ...
        for (String propertyName : properties.stringPropertyNames()) {
            ...
            if (!propertyNames.contains(propertyName)) {
				LoggingUtils.getSmartUtLogger().error("* Unknown property: " + propertyName);
	...
	public static void handleClassPath(CommandLine line) {
		...
		if (line.hasOption("smartutCP")) {
			String entry = line.getOptionValue("smartutCP");
			String[] entries = entry.split(File.pathSeparator);
			ClassPathHandler.getInstance().setSmartUtClassPath(entries);
		}
	}
