...
import org.smartut.ConsoleProgressBar;
...
public class ExternalProcessGroupHandler {
	...
	public boolean startProcessGroup(List<String[]> commands) {
		...
		for (int i = 0; i < commands.size(); i++) {
			...
			hsErrFiles[i] = "hs_err_SmartUt_client_p" + getServerPort() + "_t" + System.currentTimeMillis();
	...
	protected boolean startProcess(String[] command, int processIndex, Object population_data) {
		...
		if (!Properties.CLIENT_ON_THREAD) {
			 ...
			 * JUnit files created for testing SmartUt.
			 */
			...
			public void run() {
				...
				while (read && !isInterrupted()) {
					...
					if (message.equals(Messages.FINISHED_COMPUTATION)) {
						LoggingUtils.getSmartUtLogger().info("* Computation finished");
						read = false;
						killProcess(processIndex);
						final_result = data;
						...
						LoggingUtils.getSmartUtLogger().info("* Restarting client process");
						killProcess(processIndex);
						/*
