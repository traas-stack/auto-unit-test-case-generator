...
/**
 * Job executor will run SmartUt on separate processes.
...
public class JobExecutor {
	...
	/**
	 * Do a separate search with SmartUt for all jobs in the given list.
	...
	public synchronized void executeJobs(final List<JobDefinition> jobs, final int cores) throws IllegalStateException{
		...
		Thread mainThread = new Thread(){
			...
			public void run(){
				...
				try{
					LoggingUtils.getSmartUtLogger().info("Going to execute "+jobs.size()+" jobs");

					long minutes = configuration.timeInMinutes;
					LocalDateTime endBy = LocalDateTime.now().plus(minutes, ChronoUnit.MINUTES);
					LoggingUtils.getSmartUtLogger().info("Estimated completion time: "+minutes+" minutes, by "+endBy);
	...
	public void doneWithJob(JobDefinition job){
		...
		latch.countDown();
		LoggingUtils.getSmartUtLogger().info("Completed job. Left: "+latch.getCount());
	}
