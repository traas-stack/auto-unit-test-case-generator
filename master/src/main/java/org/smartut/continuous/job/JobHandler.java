...
public class JobHandler extends Thread {
	...
	public void run() {
		while (!this.isInterrupted()) {
			...
			try {
				...
				LoggingUtils.getSmartUtLogger().info("Going to start job for: " + job.cut +
	...
	private List<String> getCommandString(JobDefinition job) {
		...
		/*
		 *  it is important to set it before calling SmartUt, as it has to be read by Master before loading properties.
		...
		commands.add("-Dsmartut.log.folder=" + logs.getAbsolutePath() + File.separator + job.cut);
		...
		/*
			Actual call to SmartUt. "Commands" before this line will be applied
			to the spawn process, whereas the ones after will be its input parameters
		 */
		commands.add(org.smartut.SmartUt.class.getName());
		...
		commands.add("-Denable_asserts_for_smartut=" + Properties.ENABLE_ASSERTS_FOR_SMARTUT);
