  ...
  protected List<JobDefinition> createScheduleOnce() {
    ...
    for (ClassInfo c_info : classesInfo) {
      ...
      if (!c_info.hasChanged() && !c_info.isToTest()) {
        LoggingUtils.getSmartUtLogger().info("- Skipping class " + c_info.getClassName()
      ...
      if ((totalBudgetUsed + budget) <= totalBudget) {
        ...
        // classes that SmartUt failed to generate test cases and classes that
    ...
    if (totalLeftOver > 0 && totalLeftOver < totalBudget) {
      LoggingUtils.getSmartUtLogger().info("Distributing left budget (" + totalLeftOver + ")");
