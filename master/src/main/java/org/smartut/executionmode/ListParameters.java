...
import org.smartut.Properties.Parameter;
...
public class ListParameters {
	...
	public static Object execute(){
		...
		/*
		 * This is necessary, as reading from smartut-files properties
		...
		LoggingUtils.getSmartUtLogger().info(name + getGap(name,maxName) + space + type + getGap(type,maxType) + 
				space + defaultValue + getGap(defaultValue,maxDefault) + space + description);

		for(Row row : rows){
			LoggingUtils.getSmartUtLogger().info(row.name + getGap(row.name,maxName) + space + row.type + getGap(row.type,maxType) + 
