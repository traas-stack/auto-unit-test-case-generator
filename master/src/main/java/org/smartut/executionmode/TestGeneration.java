...
import org.smartut.rmi.service.ClientNodeRemote;
import org.smartut.runtime.util.JarPathing;
import org.smartut.runtime.util.JavaExecCmdUtil;
import org.smartut.statistics.SearchStatistics;
import org.smartut.utils.ExternalProcessGroupHandler;
...
public class TestGeneration {
	...
	public static List<List<TestGenerationResult>> executeTestGeneration(Options options, List<String> javaOpts,
		...
		if(cp==null || cp.isEmpty()){
			LoggingUtils.getSmartUtLogger().error("No classpath has been defined for the target project.\nOn the command line you can set it with the -projectCP option\n");
            Help.execute(options);
			return results;
		}
		...
		if (line.hasOption("class")) {
			...
			LoggingUtils.getSmartUtLogger().error(
	...
	private static List<List<TestGenerationResult>> generateTestsLegacy(Properties.Strategy strategy,
		...
		LoggingUtils.getSmartUtLogger().info("* Using .task files in "
	...
	private static List<List<TestGenerationResult>> generateTestsPrefix(Properties.Strategy strategy, String prefix,
		...
		try {
			if (Properties.INSTRUMENT_CONTEXT||Properties.INHERITANCE_FILE.isEmpty()) {
				String inheritanceFile = SmartUt.generateInheritanceTree(cp);
				args.add("-Dinheritance_file=" + inheritanceFile);
			}
		} catch (IOException e) {
			LoggingUtils.getSmartUtLogger().info("* Error while traversing classpath: " + e);
			return results;
		}
		LoggingUtils.getSmartUtLogger().info("* Found " + classes.size()
		...
		for (String sut : classes) {
			try {
				if (ResourceList.getInstance(TestGenerationContext.getInstance().getClassLoaderForSUT()).isClassAnInterface(sut)) {
					LoggingUtils.getSmartUtLogger().info("* Skipping interface: "+sut);
					continue;
				}
			} catch (IOException e) {
				LoggingUtils.getSmartUtLogger().info("Could not load class: " + sut);
				continue;
			}
			LoggingUtils.getSmartUtLogger().info("* Current class: "+ sut);
			results.addAll(generateTests(Strategy.SmartUt,sut,args));
		}
		return results;
	}
	...
	private static boolean findTargetClass(String target) {
		...
		LoggingUtils.getSmartUtLogger().info("* Unknown class: " + target +
	...
	private static List<List<TestGenerationResult>> generateTests(Properties.Strategy strategy, String target,
		...
		LoggingUtils.getSmartUtLogger().info("* Going to generate test cases for class: "+target);
		...
		if (!BytecodeInstrumentation.checkIfCanInstrument(target)) {
			throw new IllegalArgumentException(
			        "Cannot consider "
			                + target
			                + " because it belongs to one of the packages SmartUt cannot currently handle");
		}

        final String DISABLE_ASSERTIONS_SMARTUT = "-da:"+PackageInfo.getSmartUtPackage()+"...";
        final String ENABLE_ASSERTIONS_SMARTUT = "-ea:"+PackageInfo.getSmartUtPackage()+"...";
        ...
        handler.setBaseDir(SmartUt.base_dir_path);
		...
		String logDir = System.getProperty("smartut.log.folder");
		if(logDir!=null){
			// this parameter is for example used in logback-ctg.xml
			cmdLine.add(" -Dsmartut.log.folder="+logDir);
		}
		//------------------------------------------------
		...
		if (!Properties.PROFILE.isEmpty()) {
			...
			if(!agentFile.exists()) {
				LoggingUtils.getSmartUtLogger().info("* Error: "+Properties.PROFILE+" not found");
			} else {
				cmdLine.add("-agentpath:" + Properties.PROFILE);
				LoggingUtils.getSmartUtLogger().info("* Using profiling agent " + Properties.PROFILE);
			}
		}
        ...
        for (String entry : ClassPathHandler.getInstance().getTargetProjectClasspath().split(File.pathSeparator)) {
            try {
                ...
                LoggingUtils.getSmartUtLogger().info("* Error while adding classpath entry: "
                        + entry);
            }
        }
        ...
        for (int i = 0; i < Properties.NUM_PARALLEL_CLIENTS; i++) {
            ...
            if (i == 0 && Properties.DEBUG) {
                ...
                cmdLineClone.add("-Xrunjdwp:transport=dt_socket,server=y,suspend=y,address="
                        + Properties.PORT);
                LoggingUtils.getSmartUtLogger().info("* Waiting for remote debugger to connect on port "
                        + Properties.PORT + "...");
            }
            ...
            for (String s : cmdLineClone) {
                // first check client
                if (s.startsWith("-Denable_asserts_for_smartut")) {
                    if (s.endsWith("false")) {
                        definedEAforClient = DISABLE_ASSERTIONS_SMARTUT;
                    } else if (s.endsWith("true")) {
                        definedEAforClient = ENABLE_ASSERTIONS_SMARTUT;
                    }
                }
                // then check SUT
            ...
            if (definedEAforClient == null) {
                if (Properties.ENABLE_ASSERTS_FOR_SMARTUT) {
                    definedEAforClient = ENABLE_ASSERTIONS_SMARTUT;
                } else {
                    definedEAforClient = DISABLE_ASSERTIONS_SMARTUT;
                }
             ...
             */
            if (definedEAforClient.equals(ENABLE_ASSERTIONS_SMARTUT)) {
            ...
            if (!Properties.CLIENT_ON_THREAD) {
                ...
                cmdLineClone.add(1, "-Dsmartut.log.appender=CLIENT");
            }
		...
		if (handler.startProcessGroup(processArgs)) {
			...
			LoggingUtils.getSmartUtLogger().info("* Could not connect to client process");
		}
	...
	private static void handleClassPath(List<String> cmdLine) {
		String classPath = ClassPathHandler.getInstance().getSmartUtClassPath();
