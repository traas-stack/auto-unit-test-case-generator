 ...
 */
package org.smartut.jenkins.scm;
...
public class Git implements SCM {
	...
	public Git(GitSCM gitSCM, AbstractBuild<?, ?> build, BuildListener listener)
		...
		listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Remote config " + remoteConfig.toString());
		...
		if (credentialID == null || credentialID.equals("null")) {
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "No credentials defined.");
	...
	public int commit(AbstractMavenProject<?, ?> project, AbstractBuild<?, ?> build,
		...
		try {
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Commiting new test cases");

			Set<String> branches = this.getBranches();
			if (!branches.contains(branchName)) {
				// create a new branch called "smartut-tests" to commit and
				// push the new generated test suites
				listener.getLogger()
						.println(SmartUtRecorder.LOG_PREFIX + "There is no branch called " + branchName);
			...
			try {
		        ...
		        for (MavenModule module : prj.getModules()) {
    				String status = ((CliGitAPIImpl) this.gitClient).launchCommand("ls-files", "--deleted", "--modified",
    						...
    						ctgBestsDir);
    				listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Status (" + status.length() + "):\n" + status);
    				filesToBeCommitted.append(status);
		        }

		        String s_filesToBeCommitted = filesToBeCommitted.toString();
				if (s_filesToBeCommitted.isEmpty()) {
					listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Nothing to commit");
					return 0;
				}
				...
				for (String toCommit : s_filesToBeCommitted.split("\\R")) {
					...
					if (new File(filePath).exists()) {
						listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "adding: " + filePath);
						this.gitClient.add(filePath);

						number_of_files_committed++;
					} else {
						listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "File '" + filePath + "' reported by git status command does not exist");
					}
				}
				...
				// launchCommand method. as a workaround, we can simple add all files under .smartut/best-tests
				...
				listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + e.getMessage() + "\nTrying a different approach!");
				...
				for (FilePath fileToCommit : filesToCommit) {
					listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "adding: " + fileToCommit.getRemote());
			...
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + commit_msg);
			this.gitClient.commit(commit_msg);

			return number_of_files_committed;

		} catch (InterruptedException | IOException e) {
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Commit failed " + e.getMessage());
			e.printStackTrace();
	...
	public boolean push(AbstractBuild<?, ?> build, BuildListener listener, String branchName) {
		try {
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Pushing new test cases");
	...
	public void rollback(AbstractBuild<?, ?> build, BuildListener listener) {
		try {
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Rollback, cleaning up workspace");
