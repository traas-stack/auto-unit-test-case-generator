...
public class Mercurial implements SCM {
	...
	public Mercurial(MercurialSCM mercurialSCM, AbstractMavenProject<?, ?> project, AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
		...
		listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Credentials " + credentials.getDescription());
		...
		try {
			...
			if (!branches.contains(branchName)) {
				...
				if (this.hgClient.run("branch", branchName).pwd(build.getWorkspace()).join() != 0) {
					listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Unable to create a new branch called " + branchName);
					return -1;
				}
			}

			// switch to SMARTUT_BRANCH
			if (this.hgClient.run("update", branchName).pwd(build.getWorkspace()).join() != 0) {
				listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Unable to switch to branch " + branchName);
				return -1;
			}
