 ...
 */
package org.smartut.jenkins.recorder;

import org.smartut.jenkins.actions.BuildAction;
import org.smartut.jenkins.actions.ProjectAction;
import org.smartut.jenkins.scm.Git;
import org.smartut.jenkins.scm.Mercurial;
...
public class SmartUtRecorder extends Recorder {

	public static final String LOG_PREFIX = "[SmartUt] ";
	...
	public SmartUtRecorder(boolean disableAutoCommit, boolean disableAutoPush,
	...
	public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener) throws InterruptedException, IOException {
		...
		if (build.getResult().isWorseThan(Result.SUCCESS)) {
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Build did not succeed, so no test case generation by SmartUt will occur.");
			return true;
		}
		...
		if (scm == null) {
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "Project '" + project.getName() + "' has no Source-Control-Management (SCM) defined.");
			return true;
		}

		org.smartut.jenkins.scm.SCM scmWrapper = null;
		...
		if (scm instanceof MercurialSCM) {
			...
			listener.getLogger().println(SmartUtRecorder.LOG_PREFIX + "SCM of type " + scm.getType() + " not supported!");
			return true;
		}
	...
	public static class DescriptorImpl extends BuildStepDescriptor<Publisher> {
		...
		public Publisher newInstance(StaplerRequest req, JSONObject formData)
			...
			return (SmartUtRecorder) super.newInstance(req, formData);
		}
	}
}
