...
import org.smartut.result.BranchInfo;
import org.smartut.result.Failure;
...
public class MarkerWriter {
	...
	public static void clearMarkers(IResource res) throws CoreException {
		if (res.exists()) {
			res.deleteMarkers("SmartUtQuickFixes.exceptionmarker", true, 1);
			res.deleteMarkers("SmartUtQuickFixes.notcoveredmarker", true, 1);
			res.deleteMarkers("SmartUtQuickFixes.uncoveredlinemarker", true, 1);
			res.deleteMarkers("SmartUtQuickFixes.lineremovedmarker", true, 1);
		} else
					...
					public void run() {
						...
						if (store.getBoolean("removed")) {
							for (Integer i : lines) {
								...
								if (marker) {
									try {
										IMarker m = res
												.createMarker("SmartUtQuickFixes.lineremovedmarker");
						...
						if (store.getBoolean("uncovered")) {
							for (Integer i : tgr.getUncoveredLines()) {
								...
								if (marker) {
									try {
										IMarker m = res
												.createMarker("SmartUtQuickFixes.uncoveredlinemarker");
										m.setAttribute(IMarker.LINE_NUMBER, i);
										m.setAttribute(IMarker.PRIORITY,
												IMarker.PRIORITY_HIGH);
										m.setAttribute(IMarker.SEVERITY,
						...
						for (BranchInfo bi : tgr.getUncoveredBranches()) {
							...
							if (marker) {
								try {
									IMarker m = res
											.createMarker("SmartUtQuickFixes.notcoveredmarker");
									m.setAttribute(
											IMarker.MESSAGE,
											"This branch (starting line "
													+ j
													+ ") could not be covered by SmartUt.");
									...
									m.setAttribute(IMarker.LOCATION,
											res.getName());
									m = res.createMarker("SmartUtQuickFixes.uncoveredlinemarker");
									// m.setAttribute(IMarker.LINE_NUMBER, j);
									m.setAttribute(IMarker.PRIORITY,
											IMarker.PRIORITY_HIGH);
									m.setAttribute(IMarker.SEVERITY,
								...
								try {
									for (Failure f : failures) {
										if (f != null) {
											...
											if (marker) {
												IMarker m = res
														.createMarker("SmartUtQuickFixes.exceptionmarker");
												m.setAttribute(IMarker.MESSAGE,
														f.getExceptionName()
																+ " detected "
																+ message);
			...
			if (parent instanceof IAnnotatable) {
				...
				try {
					for (IAnnotation a : p.getAnnotations()) {
						if (a.getElementName().equalsIgnoreCase("SmartUtIgnore")) {
							return false;
						}
					}
				} catch (JavaModelException e) {
