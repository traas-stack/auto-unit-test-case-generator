 ...
 * <p>
 * 
 * On each execution of SmartUt-Maven-Plugin this class checks which
 * files have changed since SmartUt was invoked the last time
 * 
 ...
 * 
 * therefore, when SmartUt-Maven-Plugin is executed with HistorySchedule
...
public class HistoryChanges {
	...
	public static void keepTrack(String basedir, List<File> files) throws Exception {
		...
		File dot_smartut = new File(basedir + File.separator + Properties.CTG_DIR);
		if (!dot_smartut.exists()) {
			if (!dot_smartut.mkdir()) {
