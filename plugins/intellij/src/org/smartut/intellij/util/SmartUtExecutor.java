...
import org.smartut.intellij.EvoParameters;
...
/**
 * Singleton used to run SmartUt Maven plugin on a background process
 * <p/>
 * Created by arcuri on 9/29/14.
 */
public class SmartUtExecutor {

    private static SmartUtExecutor singleton = new SmartUtExecutor();
    ...
    private SmartUtExecutor() {
    }

    public static SmartUtExecutor getInstance() {
        return singleton;
    }
    ...
    public synchronized void run(final Project project, final EvoParameters params, final Map<String, Set<String>> suts, final AsyncGUINotifier notifier)
        ...
        if (isAlreadyRunning()) {
            throw new IllegalStateException("SmartUt already running");
        }


        Task.Backgroundable task = new EvoTask(project,"SmartUt",true,null,suts,notifier,params);
    ...
    private void executeOnAllModules(Map<String, Set<String>> suts, Project project, AsyncGUINotifier notifier,
        ...
        for (String modulePath : suts.keySet()) {
            ...
            int res = p.exitValue();
            if (res != 0) {
                notifier.failed("SmartUt ended abruptly");
                return;
            }
        }
        VirtualFileManager.getInstance().asyncRefresh(null);
        notifier.success("SmartUt run is completed");
    }
