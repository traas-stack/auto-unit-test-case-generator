 ...
 */
public class SmartUtAction extends AnAction {

    public SmartUtAction() {
        super("Run SmartUt",
                "Open GUI dialog to configure and start running SmartUt to generate JUnit tests automatically",
                loadIcon());
    }

    private static Icon loadIcon(){
        try {
            Image image = ImageIO.read( SmartUtAction.class.getClassLoader().getResourceAsStream( "smartut.png" ));
    ...
    public void actionPerformed(AnActionEvent event) {
        ...
        String title = "SmartUt Plugin";
        Project project = event.getData(PlatformDataKeys.PROJECT);

        ToolWindowManager toolWindowManager = ToolWindowManager.getInstance(project);
        ToolWindow toolWindow = toolWindowManager.getToolWindow("SmartUt");

        final AsyncGUINotifier notifier = IntelliJNotifier.getNotifier(project);

        if (SmartUtExecutor.getInstance().isAlreadyRunning()) {
            Messages.showMessageDialog(project, "An instance of SmartUt is already running",
        ...
        dialog.initFields(project, SmartUtParameters.getInstance());
        ...
        if (dialog.isWasOK()) {
            ...
            SmartUtExecutor.getInstance().run(project,EvoParameters.getInstance(),map,notifier);
        }
    }
