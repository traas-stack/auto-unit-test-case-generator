The IntelliJ plugin for SmartUt is not built as part of Maven compilation.
...
Note: the IntelliJ plugin has _NO_ compilation dependency on SmartUt. To use SmartUt, it does
call the maven plugin from a spawn process, eg "mvn smartut:generate".
Unless there are changes in the maven plugin options, there is no need to make a new IntelliJ
release even when new SmartUt releases are made.
...
to just let the build process to generate the test cases (eg with the Maven plugin for SmartUt).
