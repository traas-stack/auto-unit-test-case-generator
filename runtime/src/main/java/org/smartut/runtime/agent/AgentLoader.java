...
public class AgentLoader {
	...
	public synchronized static void loadAgent() throws RuntimeException{
		...
		if(jarFilePath==null){
			throw new RuntimeException("Cannot find either the compilation target folder nor the SmartUt jar in classpath: "+System.getProperty("java.class.path"));
		} else {
	...
	private static boolean isSmartUtMainJar(String path) throws IllegalArgumentException{
		...
		if(name.toLowerCase().contains("smartut") && name.endsWith(".jar")){
    ...
    private static String searchInAClassPath(String classPath){
        ...
        for(String entry : tokens){
			...
			String jar = findSmartUtMainJar(entry);
			if(jar != null){
				return jar;
			}
        }
	...
	private static String searchInCurrentClassLoaderIfUrlOne() {
			...
			if(loader instanceof URLClassLoader){
				...
				for(URL url : urlLoader.getURLs()){
					try {
						...
						String jar = findSmartUtMainJar(path);
						if(jar != null){
							return jar;
						}
					} catch (Exception e) {
		...
		String msg = "Failed to find SmartUt jar in current classloader. URLs of classloader:";
		for(URI uri : uris){
			msg += "\n"+uri.toString();
		}
		logger.warn(msg);
