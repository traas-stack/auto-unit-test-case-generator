...
public class MSecurityManager extends SecurityManager {
	...
	static{
		File tmp = null;
		try {
			tmp = File.createTempFile("SmartUtTmpFile", ".tmp");
			tmp.deleteOnExit();
		} catch (Exception e) {
	...
	/**
	 * Is SmartUt executing a test case?
	 */
	private volatile boolean executingTestCase;


	/**
	 * Data structure containing all the (SmartUt) threads that do not need to
	 ...
	 * but only if coming from SmartUt (and not from SUT)
	 */
	...
	/**
	 * It can happen that SmartUt encounters permissions it does not recognize.
	 * This could be due to a bug in SmartUt, or a custom permission of the SUT.
	...
	/**
	 * <p>
	 * When we start SmartUt, quite a few other threads could start as well
	...
	public void apply() throws IllegalStateException {
		try {
			...
			// this should never happen in SmartUt, ie this object should be created just once
	...
	public void goingToEndTestCase() throws IllegalStateException {
		 ...
		 * SmartUt as well
		 */
		org.smartut.runtime.System.restoreProperties();
	...
	/**
	 * Add a thread to the list of privileged thread. This is useful if SmartUt
	...
	public void checkPermission(Permission perm) throws SecurityException {
		...
		if (!allowPermission(perm)) {
			...
			if (executingTestCase) {
				/*
				 * report statistics only during test case execution, although still log them. The reason is to avoid SmartUt threads which might not
	...
	private boolean allowPermission(Permission perm) {
		...
		if(checkIfSmartUtRMI(perm) || checkIfRMIDuringTests(perm)) {
			return true;
		}

		// first check if calling thread belongs to SmartUt rather than the SUT
		if (privilegedThreads.contains(Thread.currentThread())) {

			//it is an SmartUt thread but, in special occasions, we might want to ignore its privileged status
		...
		/*
		 * Note: we had to remove this check, as some SmartUt-RMI threads would be blocked by it
		 * 
		if (!executingTestCase) {

			 // Here, the thread is not "privileged" (either from SUT or an un-registered by SmartUt), and we are not executing a test case (if from
	 ...
	 * allow only what the SmartUt client actually requests.
	 * It is not bullet-proof, but should be fine for now.
	 * </p>
	 * @param perm
	 * @return
	 */
	private boolean checkIfSmartUtRMI(Permission perm) {
		...
		if(perm instanceof FilePermission && !perm.getActions().equals("read")) {
			...
			logger.error("SmartUt RMI is trying to interact with files: "+perm);
			return false;
		}
	...
	public boolean checkIfRMIDuringTests(Permission perm) {
		...
		/*
		 * if we are running test cases to debug SmartUt, we always want to allow RMI.
	 ...
	 * can issue some warnings, and might make SmartUt more difficult to port and use on different OS,installations, or even Java versions
	 */
	...
	// -----------------------------------------------------------------------------
	/*
	 * SmartUt does not use any bean, so allowing the SUT to create/use beans should be fine. However, there is possible issue of beans created by a
	...
	protected boolean checkSQLPermission(SQLPermission perm) {
		/*
		 * SQL (and database in general) will require specialized techniques in SmartUt. For now, we just forbid it
		 */
		return false;
	}
	...
	protected boolean checkAWTPermission(AWTPermission perm) {
		/*
		 * For now, we run SmartUt in headless mode (ie no support for display, mouse, keyboard, etc). Methods that will need those devices will
	...
	protected boolean checkRuntimePermission(RuntimePermission perm) {
		 ...
		 * So, if SmartUt already set a factory, then the SUT cannot change it.
		 * On the other hand, if the SUT set a factory, it will have potential
		 * impact on SmartUt code (eg, if SmartUt will use new Socket/URLs after
		...
		/*
		 * it might be considered risky, as it can stop the SmartUt threads. Worst case, we ll get no data from client, which is better than just
		...
		if(!unrecognizedPermissions.contains(perm)) {
			unrecognizedPermissions.add(perm);
			logger.warn("SUT asked for a runtime permission that SmartUt does not recognize: " + name);
		}
	...
	protected boolean checkPropertyPermission(PropertyPermission perm) {
		...
		if(perm.getActions().contains("write") && !executingTestCase) {
			if(org.smartut.runtime.System.isSystemProperty(perm.getName())) {
				return false;
			} else {
				return true;
			}
		...
		return org.smartut.runtime.System.handlePropertyPermission(perm);
	}
	...
	protected boolean checkFilePermission(FilePermission fp) {
		 ...
		 * SmartUt client is reading, then that deletion would be forbidden, as client has lock.
		 */
		...
		if(action.equals("write")) {
			 ...
			 */
				...
				// As user.dir might not exist, SmartUt must grant access write access to
				// GZoltar.
