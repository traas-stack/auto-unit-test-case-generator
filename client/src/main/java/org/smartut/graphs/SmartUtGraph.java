 ...
 * SmartUt Examples are the raw and minimal Control Flow Graph and hopefully at
 * one point the Control Dependency Tree
 *
 * This class is supposed to hide the jGraph library from the rest of SmartUt
 ...
 * files in SmartUt that import anything from the jGraph library - at least
 ...
 *
 * For now an SmartUtGraph can always be represented by a DefaultDirectedGraph
 ...
 * @author Andre Mis
 */
public abstract class SmartUtGraph<V, E extends DefaultEdge> {

	private static final Logger logger = LoggerFactory.getLogger(SmartUtGraph.class);

	private static int smartUtGraphs = 0;
	...
	/**
	 * <p>Constructor for SmartUtGraph.</p>
	 *
	 * @param edgeClass a {@link java.lang.Class} object.
	 */
	protected SmartUtGraph(Class<E> edgeClass) {
	 ...
	 * @param edgeClass a {@link java.lang.Class} object.
	 */
	protected SmartUtGraph(DirectedGraph<V, E> graph, Class<E> edgeClass) {
	...
	public Set<V> getChildren(V node) {
		if (!containsVertex(node)){
			LoggingUtils.getSmartUtLogger().warn("getChildren call requests a node not contained in the current graph. Node: "+node);
			return null;
		}
	 ...
	 * <p>addVertices</p>
	 *
	 * @param other a {@link org.smartut.graphs.SmartUtGraph} object.
	 */
	protected void addVertices(SmartUtGraph<V, E> other) {
	...
	private String getGraphDirectory() {
		return "smartut-graphs/" + dotSubFolder();
	}
