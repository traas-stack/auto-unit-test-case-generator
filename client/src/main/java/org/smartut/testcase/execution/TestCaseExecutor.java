...
public class TestCaseExecutor implements ThreadFactory {
	 ...
	 * </p>
	 * 
	 * @return a {@link org.smartut.testcase.execution.TestCaseExecutor}
	 *         object.
	 */
	 ...
	 * 
	 * @param observer
	 *            a {@link org.smartut.testcase.execution.ExecutionObserver}
	 *            object.
	 */
	...
	private ExecutionResult execute(TestCase tc, Scope scope, int timeout) {
		...
		try {
			 ...
			 * smartut. As exceptions from the tested code are caught before
			 * this.
			 */
	...
	private boolean isInStaticInit() {
		for (StackTraceElement elem : currentThread.getStackTrace()) {
			...
			if (elem.getMethodName().equals("loadClass") && elem.getClassName()
					.equals(org.smartut.instrumentation.InstrumentingClassLoader.class.getCanonicalName()))
				return true;
