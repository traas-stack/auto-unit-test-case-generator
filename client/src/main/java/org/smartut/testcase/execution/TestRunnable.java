...
import org.smartut.runtime.System.SystemExitException;
import org.smartut.runtime.jvm.ShutdownHookHandler;
import org.smartut.runtime.thread.KillSwitch;
...
import org.smartut.dse.VMError;
...
public class TestRunnable implements InterfaceTestRunnable {
	...
	public ExecutionResult call() {
		...
		try {
			...
			if (e instanceof AssertionError
			        && e.getStackTrace()[0].getClassName().contains(PackageInfo.getSmartUtPackage())) {
				logger.error("Assertion Error in smartutcode, for statement \n"
		...
		result.setReadProperties(org.smartut.runtime.System.getAllPropertiesReadSoFar());
		result.setWasAnyPropertyWritten(org.smartut.runtime.System.wasAnyPropertyWritten());
	...
	private void executeStatements(ExecutionResult result, PrintStream out,
			...
			InstantiationException, VMError, SmartUtError {
		...
		for (Statement s : test) {
			...
			if (exceptionThrown != null) {
				...
				if (exceptionThrown instanceof SmartUtError) {
					throw (SmartUtError) exceptionThrown;
				}
				// -------------------------------------------------------
