...
import org.smartut.runtime.*;
import org.smartut.runtime.System;
import org.smartut.runtime.annotation.*;
...
/**
 *
 * This class is responsible to augment {@link org.smartut.setup.TestCluster}
...
public class EnvironmentTestClusterAugmenter {
	...
	/**
	 * Keep track of all SmartUt classes that have been already fully handled
	 * (via recursion)
	 */
	 ...
	 *
	 * @see org.smartut.runtime.Random
	 * @see org.smartut.runtime.System
	 */
	 ...
	 * modifiers) all the other SmartUt classes for which the given class is a
	 * generator
	 *
	 * @param klass
	 */
	private boolean addEnvironmentClassToCluster(Class<?> klass) {
		if (handledClasses.contains(klass.getCanonicalName()) || !TestClusterUtils.isSmartUtClass(klass)) {
			return false; // already handled, or not valid
		}
		handledClasses.add(klass.getCanonicalName());

		boolean excludeClass = klass.getAnnotation(SmartUtClassExclude.class) != null;
	...
	private boolean isObjectMethod(AccessibleObject ao) {
		 ...
		 * However, as we only apply it on SmartUt methods, should be fine
		 */
	...
	private boolean shouldSkip(boolean excludeClass, AccessibleObject c) {
		...
		if (excludeClass) {
			boolean include = c.getAnnotation(SmartUtInclude.class) != null;
			if (!include) {
				return true;
			}
		} else {
			boolean exclude = c.getAnnotation(SmartUtExclude.class) != null
					|| c.getAnnotation(SmartUtAssertionOnly.class) != null;
			if (exclude) {
				return true;
			}
		}
				...
				TestCluster.getInstance()
						.addEnvironmentTestCall(new GenericMethod(
								NetworkHandling.class.getMethod("createRemoteTextFile",
										new Class<?>[] { SmartUtURL.class, String.class }),
						...
						.addEnvironmentTestCall(
								new GenericMethod(
										NetworkHandling.class.getMethod("sendUdpPacket",
												new Class<?>[] { SmartUtLocalAddress.class,
														SmartUtRemoteAddress.class, byte[].class }),
										new GenericClass(NetworkHandling.class)));
				TestCluster.getInstance()
						.addEnvironmentTestCall(new GenericMethod(
								NetworkHandling.class.getMethod("sendUdpPacket",
										new Class<?>[] { SmartUtLocalAddress.class, byte[].class }),
								new GenericClass(NetworkHandling.class)));
			} catch (Exception e) {
				...
				TestCluster.getInstance()
						.addEnvironmentTestCall(new GenericMethod(
								NetworkHandling.class.getMethod("sendMessageOnTcp",
										new Class<?>[] { SmartUtLocalAddress.class, String.class }),
								new GenericClass(NetworkHandling.class)));
			} catch (Exception e) {
				...
				TestCluster.getInstance()
						.addEnvironmentTestCall(new GenericMethod(
								NetworkHandling.class.getMethod("openRemoteTcpServer",
										new Class<?>[] { SmartUtRemoteAddress.class }),
								new GenericClass(NetworkHandling.class)));
			} catch (Exception e) {
	...
	private void handleVirtualFS(TestCase test) {
		...
		if (!hasAddedFiles && VirtualFileSystem.getInstance().getAccessedFiles().size() > 0) {
			logger.info("Adding SmartUtFile calls to cluster");
	...
	private void handleReplaceCalls() {
		...
		if (!hasAddedSystem && org.smartut.runtime.System.wasTimeAccessed()) {
			hasAddedSystem = true;
			try {
				cluster.addTestCall(
