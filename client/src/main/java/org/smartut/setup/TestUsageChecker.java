...
import org.smartut.utils.Java9InvisiblePackage;
...
public class TestUsageChecker {
    ...
    public static boolean canUse(Class<?> c) {
        ...
        if (!Properties.USE_DEPRECATED) {
	        try {
	            ...
	            AtMostOnceLogger.warn(LoggingUtils.getSmartUtLogger(), "ArrayStoreException caught while handling class "+c.getName());
                AtMostOnceLogger.warn(LoggingUtils.getSmartUtLogger(), "This is likely due to a missing dependency used as annotation: https://bugs.java.com/view_bug.do?bug_id=JDK-7183985");
	            return false;
            }
        }
        ...
        if (TestClusterUtils.isSmartUtClass(c) && !MockList.isAMockClass(c.getCanonicalName())) {
            return false;
        }
    ...
    public static boolean canUse(Method m, Class<?> ownerClass) {
        ...
        if (m.isAnnotationPresent(SmartUtTest.class)) {
            logger.debug("Excluding SmartUt test method " + m.getName());
            return false;
        }

        if (m.isAnnotationPresent(SmartUtExclude.class)) {
        ...
        // FIXME: SmartUt currently can't deal properly with the Map.of(...) methods introduced in Java 9
        if(m.getDeclaringClass().equals(java.util.Map.class) && Modifier.isStatic(m.getModifiers())) {
            return false;
        }

        // FIXME: SmartUt currently can't deal properly with the Set.of(...) methods introduced in Java 9
		...
		if(m.getTypeParameters().length > 0) {
			...
			if(m.getDeclaringClass().equals(Properties.getTargetClass())) {
				LoggingUtils.getSmartUtLogger().info("* Skipping method "+m.getName()+": generic methods are not handled yet");
			}
			return false;
		}
		*/
