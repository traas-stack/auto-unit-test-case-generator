...
public class InheritanceTreeGenerator {
	...
	public static InheritanceTree createFromClassPath(List<String> classPath) {
		if (!Properties.INSTRUMENT_CONTEXT && !Properties.INHERITANCE_FILE.isEmpty()) {
			try {
				...
				LoggingUtils.getSmartUtLogger().warn("* " + ClientProcess.getPrettyPrintIdentifier() +
		...
		for (String classPathEntry : classPath) {
			...
			if (classPathEntry.matches(".*smartut-.*\\.jar"))
				continue;

			logger.debug("Analyzing classpath entry {}", classPathEntry);
			LoggingUtils.getSmartUtLogger().info("  - " + classPathEntry);
	...
	private static void analyzeClassStream(InheritanceTree inheritanceTree,
		...
		try {
			...
			logger.error("ASM Error while classReader reading class: ", e);
		}
	}
...
//			}
	...
	public static void generateJDKCluster(String... filters) {
		...
		try {
			...
			xstream.allowTypesByWildcard(new String[] {"org.smartut.**", "org.jgrapht.**"});
	...
	private static void makeShadedCopy(){
		...
		String shadedContent = content.replace(PackageInfo.getSmartUtPackage(), PackageInfo.getShadedSmartUtPackage());
    ...
    /*
        ...
        java -cp master/target/smartut-master-0.1.1-SNAPSHOT-jar-minimal.jar   org.smartut.setup.InheritanceTreeGenerator
