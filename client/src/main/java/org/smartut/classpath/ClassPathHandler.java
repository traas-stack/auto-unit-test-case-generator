...
/**
 * When running SmartUt there are at least three different classpaths
 * to handle: (1) the one of the target project, (2) the one of SmartUt
 * itself, and (3) the one of the starter of SmartUt.
 * Note that (2) and (3) can indeed be different: eg if SmartUt is
...
public class ClassPathHandler {
	...
	/**
	 * When we start the client, we need to know what is the classpath of SmartUt.
	 * Problem is, if SmartUt was called from Eclipse/Maven, when the classpath will
	 ...
	 */
	private String smartutClassPath;
	...
	public static void resetSingleton(){
		...
		getInstance().smartutClassPath = null;
	}

	public String getSmartUtClassPath(){
		if(smartutClassPath==null){
			smartutClassPath = System.getProperty("java.class.path");
		}
		
		return smartutClassPath;
	}
	
	/**
	 * Replace current CP of SmartUt with the given <code>elements</code>
	 * @param elements
	 * @throws IllegalArgumentException if values in <code>elements</code> are not valid classpath entries
	 */
	public void setSmartUtClassPath(String[] elements) throws IllegalArgumentException{
	...
	public static String writeClasspathToFile(String classpath) {
		...
		try {
			File file = File.createTempFile("SmartUt_classpathFile",".txt");
			file.deleteOnExit();
	...
	/**
	 * This is meant only for running the SmartUt test cases, whose CUTs will be in the
	 * classpath of SmartUt itself
	 */
	public void changeTargetCPtoTheSameAsSmartUt(){
		...
		if(outDir.exists()){
			...
			changeTargetClassPath(getSmartUtClassPath().split(File.pathSeparator));
		}
	}
}