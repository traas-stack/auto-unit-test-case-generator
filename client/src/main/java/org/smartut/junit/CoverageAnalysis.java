...
import org.smartut.utils.ExternalProcessUtilities;
...
public class CoverageAnalysis {
	 ...
	 * must have to be converted to some format that SmartUt can understand
	 */
	...
	public static void analyzeCoverage() {
		...
		LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Found " + testClasses.size()
                + " test class(es)");
	...
	private static List<Class<?>> getTestClassesFromClasspath() {
		...
		for(String prefix : Properties.JUNIT.split(":")) {
			...
			LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Found " + suts.size()
	...
	private static List<Class<?>> getTestClasses() {
		...
		for(String prefix : Properties.JUNIT.split(":")) {
			...
			LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Analyzing entry: "+prefix);
	...
	private static void analyzeCoverageCriterion(List<JUnitResult> results, Properties.Criterion criterion) {
        ...
        if (goals.isEmpty()) {
			LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Coverage of criterion "
        	...
        	LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Coverage of criterion " + criterion
                    + ": " + NumberFormat.getPercentInstance().format(coverage));
			LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Number of covered goals: "
	...
	private static void printReport(List<JUnitResult> results) {
		...
		while (it.hasNext()) {
			...
			LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Target class "
			...
			LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Total number of covered goals: "
			...
			LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Total coverage: "
                    + NumberFormat.getPercentInstance().format(coverage));
	...
	private static List<JUnitResult> executeTests(Class<?>... testClasses) {
        ...
        for (Class<?> testClass : testClasses) {
        	LoggingUtils.getSmartUtLogger().info("  Executing " + testClass.getSimpleName());
        ...
        LoggingUtils.getSmartUtLogger().info("* " + ClientProcess.getPrettyPrintIdentifier() + "Executed " + results.size() + " unit "
                + "test(s)");
	...
	public static boolean isTest(Class<?> cls) {
		...
		try {
			...
			methods.addAll(tc.getAnnotatedMethods(SmartUtTest.class));
	...
	public void run() {
		...
		LoggingUtils.getSmartUtLogger().info("* Connecting to master process on port "
