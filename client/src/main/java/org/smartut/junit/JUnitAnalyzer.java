...
public class JUnitAnalyzer {
		...
		try {
			...
			failure_loop: for (JUnitFailure failure : result.getFailures()) {
				...
				if(testName.equals("initializationError") && failure.getMessage().contains("Failed to attach Java Agent")) {
					logger.warn("Likely error with SmartUt instrumentation, ignoring failure in test execution");
					continue failure_loop;
				}
				...
				for (int i = 0; i < tests.size(); i++) {
					if (TestSuiteWriterUtils.getNameOfTest(tests, i).equals(testName)) {
						 ...
						 * we should delete it, as it would either represent a bug in SmartUt
	...
	private static JUnitResult runJUnitOnCurrentProcess(Class<?>[] testClasses) {
		...
		if(wasSandboxOn){
			...
			if(Sandbox.isSecurityManagerInitialized()){
				logger.warn("SmartUt problem: tests set up a security manager, but they do not remove it after execution");
	...
	// SmartUt classloader, and thus cannot easily be re-loaded
	...
	private static List<File> compileTests(List<TestCase> tests, File dir) {
		...
		try {
			...
			String smartutCP = ClassPathHandler.getInstance().getSmartUtClassPath();
			if(JarPathing.containsAPathingJar(smartutCP)){
				smartutCP = JarPathing.expandPathingJars(smartutCP);
			}
			...
			String classpath = targetProjectCP + File.pathSeparator + smartutCP;
			...
			if (!compiled) {
				...
				//TODO remove
				logger.error("smartutCP: "+smartutCP);
	...
	protected static File createNewTmpDir() {
		File dir = null;
		String dirName = FileUtils.getTempDirectoryPath() + File.separator + "SmartUt_"
	...
	/**
	 * <p>
	 * The output of SmartUt is a set of test cases. For debugging and
	 * experiment, we usually would not write any JUnit to file. But we still
	 * want to see if test cases can compile and execute properly. As SmartUt
	...
	public static boolean verifyCompilationAndExecution(List<TestCase> tests) {
		...
		try {
			...
			if (!result.wasSuccessful()) {
				 ...
				 * 
				 * Here we just log (and not return false), as it might be that SmartUt is just not able to generate
