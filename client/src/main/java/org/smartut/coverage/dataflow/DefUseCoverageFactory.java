...
import org.smartut.coverage.dataflow.DefUseCoverageTestFitness.DefUsePairType;
import org.smartut.coverage.dataflow.analysis.AllUsesAnalysis;
import org.smartut.graphs.GraphPool;
import org.smartut.graphs.ccfg.ClassControlFlowGraph;
...
import org.smartut.utils.JdkPureMethodsList;
...
public class DefUseCoverageFactory extends
	...
	public static void computeGoals() {
		...
		/*for(String methodInCCFG : GraphPool.getInstance(TestGenerationContext.getClassLoader()).getRawCFGs(Properties.TARGET_CLASS).keySet()) {
			if(GraphPool.getInstance(TestGenerationContext.getClassLoader()).getCCFG(Properties.TARGET_CLASS).isPure(methodInCCFG))
				LoggingUtils.getSmartUtLogger().debug("PURE method:\t"+methodInCCFG);
			else
				LoggingUtils.getSmartUtLogger().debug("IMPURE method:\t"+methodInCCFG);
		} */

		long start = System.currentTimeMillis();
		LoggingUtils.getSmartUtLogger().info("starting DefUse-Coverage goal generation");
		...
		LoggingUtils.getSmartUtLogger().info("* Creating DefUse-Pairs from CCFG...");
		duGoals.addAll(getCCFGPairs());
		LoggingUtils.getSmartUtLogger().info("  ..created " + getIntraMethodGoalsCount()
		                                         + " intra-method-, "
		...
		LoggingUtils.getSmartUtLogger().info("  "+duGoals.toString());

		LoggingUtils.getSmartUtLogger().info("* Creating parameter goals...");
		duGoals.addAll(getParameterGoals());
		LoggingUtils.getSmartUtLogger().info("  created " + getParamGoalsCount()
		                                         + " parameter goals");
		...
		LoggingUtils.getSmartUtLogger().info("* Goal computation took: "
	...
	private static void categorizeFieldMethodCalls() {
		...
		LoggingUtils.getSmartUtLogger().info("Categorizing field method calls: "
	 ...
	 * @param type
	 *            a
	 *            {@link org.smartut.coverage.dataflow.DefUseCoverageTestFitness.DefUsePairType}
	 *            object.
	 */
	 ...
	 *            object.
	 * @return a
	 *         {@link org.smartut.coverage.dataflow.DefUseCoverageTestFitness}
	 *         object.
	 */
	 ...
	 * @param use
	 *            a {@link org.smartut.coverage.dataflow.Use} object.
	 * @return a
	 *         {@link org.smartut.coverage.dataflow.DefUseCoverageTestFitness}
	 *         object.
	 */
	...
	public static Set<DefUseCoverageTestFitness> getParameterGoals() {
		...
		//		LoggingUtils.getSmartUtLogger().debug("# Parameter-Uses: " + r.size());
		return r;
	}
												...
												for (DefUseCoverageTestFitness defUse : duGoals) {
													...
													if (defUse.getGoalUse().equals(use)) {
														...
														for (Use otherUse : defUseMap.keySet()) {
															...
															DefUseCoverageTestFitness goal = createGoal(def,
															                                                                  otherUse,
