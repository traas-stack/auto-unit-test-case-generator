...
public class DefUseCoverageSuiteFitness extends TestSuiteFitnessFunction {
	...
	public static void printCoverage() {
		...
		LoggingUtils.getSmartUtLogger().info("* Time spent optimizing covered goals analysis: "
		                                         + DefUseExecutionTraceAnalyzer.timeGetCoveredGoals
		                                         + "ms");
		Map<DefUsePairType, Integer> totalGoals = initTotalGoals();
		for (DefUsePairType type : DefUseCoverageTestFitness.DefUsePairType.values()) {
			LoggingUtils.getSmartUtLogger().info("* Covered goals of type " + type + ": "
			                                         + mostCoveredGoals.get(type) + " / "
			                                         + totalGoals.get(type));
			for (DefUseCoverageTestFitness pair : getPairsOfType(type)) {
				if (pair.isCovered()) {
					LoggingUtils.getSmartUtLogger().info("*(X) " + pair.toString());
				} else {
					LoggingUtils.getSmartUtLogger().info("*( ) " + pair.toString());
				}
			}

		}

		LoggingUtils.getSmartUtLogger().info("* Covered " + countMostCoveredGoals() + "/"
