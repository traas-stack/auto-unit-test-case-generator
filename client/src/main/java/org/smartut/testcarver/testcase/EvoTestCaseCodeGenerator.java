...
import org.smartut.testcarver.codegen.ICodeGenerator;
...
import org.smartut.testcase.statements.ImmutableStringPrimitiveStatement;
...
public final class EvoTestCaseCodeGenerator implements ICodeGenerator<TestCase> {
	...
	public void createUnobservedInitStmt(CaptureLog log, int logRecNo) {
		...
		try {
			...
			final MethodStatement m = new MethodStatement(testCase, new GenericMethod(SmartUtXStream.class.getMethod("fromString", new Class<?>[] {String.class}), SmartUtXStream.class), null, Arrays.asList(stringRepRef));
