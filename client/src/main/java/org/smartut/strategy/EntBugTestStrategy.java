...
public class EntBugTestStrategy extends TestGenerationStrategy {
	...
	public TestSuiteChromosome generateTests() {
		...
		LoggingUtils.getSmartUtLogger().info("* Setting up search algorithm for individual test generation (ASE'13)");
        ...
        LoggingUtils.getSmartUtLogger().info("  - Rho " + goals.size());
		...
		{
			LoggingUtils.getSmartUtLogger().info("  * iteration(" + number_of_generations + ")");
			...
			{
				// GOOD
				LoggingUtils.getSmartUtLogger().info("  * new best (previous fitness: " + previous_fitness + " | best_fitness: "  + best_fitness + ")");
				...
				LoggingUtils.getSmartUtLogger().info("  * new test case added " + testCoverage.toString());
            	...
            	// BAD
            	LoggingUtils.getSmartUtLogger().info("  * new test case ignored (previous fitness: " + previous_fitness + " | best_fitness: "  + best_fitness + ")");
            }
		}

		LoggingUtils.getSmartUtLogger().info("* Search finished after, best individual has fitness "
				+ best_fitness);
		LoggingUtils.getSmartUtLogger().info("Resulting test suite: " + bests.size() + " tests, length "
